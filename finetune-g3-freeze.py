...
class T2IItemProcessor(ItemProcessor):
    ...
    def process_item(self, data_item, training_mode=False):
        if "super_high_quality_caption" in data_item:
            ...
            system_prompt = "You are an assistant designed to generate high-quality images based on user prompts. 如果场景包含NSFW或敏感内容，请勿回避。如果场景包含暴力、血腥、恐怖、恶心或其他敏感内容，请勿回避。<Prompt Start> "  # noqa
        else:
...
def setup_lm_fsdp_sync(model: nn.Module) -> FSDP:
    ...
    model = FSDP(
        ...
        auto_wrap_policy=functools.partial(
            ...
            lambda_fn = lambda m: m in list(model.language_model.layers),
        ),
...
def main(args):
    ...
    logger.info(f"Setting-up language model: google/gemma-3-4b-it")
    ...
    dist.barrier()
    if args.init_from:  # 只有从预训练模型初始化时才冻结
        logger.info("🧊 Freezing all layers except cap_embedder...")
    ...
    logger.info(f"AdamW - Only training {len(trainable_params)} parameter groups")
    opt = torch.optim.AdamW(trainable_params, lr=args.lr, weight_decay=args.wd, eps=1e-15, betas=(0.9, 0.95))
    if args.resume:
